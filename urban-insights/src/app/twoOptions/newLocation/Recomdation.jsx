...
export default function Recomdation(props){
    ...
    return(
        <>
        <label htmlFor='state' className='block'>{props.name}</label>
                    <div className='state py-2 w-full'>
                        <div className='state-items flex flex-row flex-wrap'>
                            {props.Array.map((value)=>{
                                return (<Btn value={value} stateArray={props.Array} changeArray={props.changeArray} />);
                            })}
                        </div>
                        <div className='w-full'>
                            <input id='state' name='state' value={value} autoComplete='off' onChange={(event)=>{
                                ...
                                for(let i=0;i<suggestion.length;i++){
                                    if(suggestion[i].toLowerCase().includes(subString.toLowerCase())){
                                        newSuggestion.push(suggestion[i])
                                    }
                                }
                                setSuggestion(newSuggestion);
                                }} className='block bg-transparent outline-none w-full' />
                            {onInput && <div className='flex flex-col absolute nform-sugg'>
